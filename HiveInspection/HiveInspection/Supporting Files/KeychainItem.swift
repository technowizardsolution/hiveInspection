...
extension KeychainItem {
    ...
    static var identityToken: String? {
        get {
            return try? KeychainItem(service: bundleIdentifier, account: "identityToken").readItem()
        }
        set {
            guard let value = newValue else {
                try? KeychainItem(service: bundleIdentifier, account: "identityToken").deleteItem()
                return
            }
            do {
                try KeychainItem(service: bundleIdentifier, account: "identityToken").saveItem(value)
            } catch {
                print("Unable to save identityToken to keychain.")
            }
        }
    }
    ...
    static var authCode: String? {
        get {
            return try? KeychainItem(service: bundleIdentifier, account: "authCode").readItem()
        }
        set {
            guard let value = newValue else {
                try? KeychainItem(service: bundleIdentifier, account: "authCode").deleteItem()
                return
            }
            do {
                try KeychainItem(service: bundleIdentifier, account: "authCode").saveItem(value)
            } catch {
                print("Unable to save identityToken to keychain.")
            }
        }
