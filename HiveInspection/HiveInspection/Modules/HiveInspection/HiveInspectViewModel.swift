...
extension HiveInspectViewModel {
    func callAPI(with model : [HiveInspectData], hiveId : String, additionalNotes : String) {
        ...
        guard let inspectionDate = model.filter({$0.title == "Date"}).first?.selectedTitle,
              ...
              let InsulateWinterize = model.filter({$0.title == "Insulate / Winterize"}).first?.selectedTitle else { return }
        ...
        let param : [String : Any] = ["data":
                                        [
                                            ...
                                            "remove_medication":RemoveMedicationSwitch ? "1" : "0",
